...
class V7000CommonTestCase(test.TestCase):
    ...
    def setup_configuration(self):
        ...
        config.pool_allocation_method = 'random'
        config.dedup_only_pools = None
        config.dedup_capable_pools = None
        return config
    ...
    def test_fail_extend_dedup_lun(self):
        ...
        size_in_mb = vol['size'] * units.Ki
        self.driver.vmem_mg = self.setup_mock_concerto()
        type(self.driver.vmem_mg.utility).is_external_head = \
            mock.PropertyMock(return_value=False)

        self.driver._get_volume_type_extra_spec = mock.Mock(
            return_value="True")

        failure = v7000_common.ViolinUnsupportedFeature
    ...
    # TBD - More delete lun failure cases to be added after collecting the
    #       possible responses from Concerto
    ...
    def test_extend_lun_new_size_is_too_small(self):
        ...
        failure = v7000_common.ViolinBackendErr
    ...
    def test_create_volume_from_snapshot(self):
        ...
        calls = [mock.call(SNAPSHOT['volume_id']),
                 mock.call(object_id=object_id)]
        self.driver.vmem_mg.lun.get_lun_info.assert_has_calls(calls)
    ...
    def test_create_volume_from_snapshot_fails(self):
        ...
        response = {'success': False,
                    'msg': 'Duplicate Virtual Device name. Error: 0x90010022'}
        compressed_snap_id = 'abcdabcd1234abcd1234abcdeffedcbb'
        failure = v7000_common.ViolinBackendErrExists
    ...
    def test_update_migrated_lun(self):
        """Lun is successfully renamed."""
        curr_vol = VOLUME.copy()
	curr_vol['id'] = "12345678"
	model_update = {'_name_id': None}
        response = {'success': True,
		    'msg': 'Update resource profile successfully'}

        conf = {
            'lun.rename_lun.return_value': response,
        }
        self.driver.vmem_mg = self.setup_mock_concerto(m_conf=conf)
        self.driver._send_cmd = mock.Mock(return_value=response)

        result = self.driver._update_migrated_volume(VOLUME, curr_vol, "True")

        self.driver._send_cmd.assert_called_with(
            self.driver.vmem_mg.lun.rename_lun,
    ...
    def test_send_cmd_request_timed_out(self):
        ...
        self.assertRaises(v7000_common.RequestRetryTimeout,
    ...
    def test_check_error_code_non_fatal_error(self):
        ...
        self.assertEqual(None, self.driver._check_error_code(response))
    ...
    def test_get_storage_pool_configured_pools(self):
        ...
        self.conf.dedup_only_pools = ['PoolA', 'PoolB']
        self.conf.dedup_capable_pools = ['PoolC', 'PoolD']
        ...
        self.driver.vmem_mg.pool.select_storage_pool.assert_called_with(
            ...
            pool_type,
            None,
            self.conf.dedup_only_pools,
            self.conf.dedup_capable_pools,
            "random",
            "create_lun",
        )
